...
function evalPrices()
{
	var prof = calc.items.profile,
		...
		koefs = {h:0,s:0,d:0};
	montazh = Math.round(w*h*1000);//монтаж длина(все в метрах)*высота*1000(тыс. руб)
	...
	skidka = 0.00;//скидка на заказ
	price = Math.ceil(price*(1-skidka));
	credit = Math.round(price/6);//сумма кредита ежемесячный платеж
	/*
